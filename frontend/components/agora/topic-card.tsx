...
export function TopicCard({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className="group w-full flex items-center gap-4 rounded-2xl border border-border/40 bg-card/60 backdrop-blur-md p-5 sm:p-6 text-left transition-all hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5"
    >
      {/* Temperature + count */}
        ...
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          ...
          {(topic.authorName || topic.authorThumb) && (
            <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
              {topic.authorThumb && (
                <span className="flex shrink-0 size-[14px] overflow-hidden rounded-full">
                  <Image
